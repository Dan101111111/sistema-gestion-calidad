...
exports.actualizarEncuesta = async (req, res, next) => {
  try {
    ...
    const { preguntas, ...datosBasicos } = req.body;
    
    if (datosBasicos.codigo && datosBasicos.codigo !== encuesta.codigo) {
      const existe = await Encuesta.findOne({ where: { codigo: datosBasicos.codigo } });
      if (existe) return next(createError(409, 'Código de encuesta duplicado'));
    }

    await encuesta.update(datosBasicos);

    if (preguntas) {
      await PreguntaEncuesta.destroy({ where: { encuesta_id: encuesta.id } });
      if (preguntas.length > 0) {
        await PreguntaEncuesta.bulkCreate(preguntas.map((p, i) => ({ 
          ...p, encuesta_id: encuesta.id, orden: p.orden ?? i 
        })));
      }
    }
...
exports.eliminarEncuesta = async (req, res, next) => {
  try {
    ...
    const respuestasCount = await RespuestaEncuesta.count({ where: { encuesta_id: encuesta.id } });
    if (respuestasCount > 0) {
      return next(createError(400, 'No se puede eliminar la encuesta porque ya tiene respuestas registradas.'));
    }

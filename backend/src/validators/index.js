...
const encuestaSchemas = {
  ...
  actualizar: Joi.object({
    codigo:         Joi.string().min(3).max(30),
    titulo:         Joi.string().min(5).max(300),
    descripcion:    Joi.string().max(3000).allow(''),
    grupo_objetivo: Joi.string().valid('estudiantes','docentes','egresados','administrativos','todos'),
    fecha_inicio:   fecha,
    fecha_fin:      fecha,
    anonima:        Joi.boolean(),
    preguntas:      Joi.array().items(Joi.object({
